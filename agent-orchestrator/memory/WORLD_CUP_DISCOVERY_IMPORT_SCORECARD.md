...
| Confidence scoring/admin review | 8 | 8 | `polymarket:admin-review:report` summarizes mapping, validation confidence, reasons, reference prices, promotion eligibility, and recommended admin action; first-class candidate review API/UI remains pending. |
...
| Total | 100 | 96 | Fixture-first discovery/import through admin review, dry-run promotion, guarded DB lifecycle mutation, local DB lifecycle E2E, imported-market MM dry-run, public no-leak checks, E2E, read-only live-smoke gating, and persisted discovery candidate storage are harnessed. |
